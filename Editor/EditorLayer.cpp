...
void EditorLayer::OnAttach()
{
	...
	myChest.GetComponent<TransformComponent>().rotation = { 0.f, -45.0f, 0.0f };
...
void EditorLayer::OnUpdate()
{
	...
	{
		myCube.GetComponent<TransformComponent>().rotation.z += 0.5f;
	}

	if (myChest.HasComponent<TransformComponent>())
	{
		myChest.GetComponent<TransformComponent>().rotation.y += 0.5f;
	}
	
	if (myPyramid.HasComponent<TransformComponent>())
	{
		myPyramid.GetComponent<TransformComponent>().rotation.y += 0.5f;
	}
}
